...
import SeacrchResult from "@/components/shared/SeacrchResult";
...
const Explore = () => {
  ...
  const { data: searchPosts, isFetching: isSearchFeatching } =
  ...
  return (
    <div className="explore-container">
      ...
      <div className="flex flex-wrap gap-9 w-full max-w-5xl">
        {shouldShowSearchResult ? (
          <SeacrchResult
            isSearchFetching={isSearchFeatching}
          ...
          posts?.pages.map((item, index) => (
            <GridPostLists key={`page-${index}`} posts={item.documents} />
          ))
        )}
      </div>
